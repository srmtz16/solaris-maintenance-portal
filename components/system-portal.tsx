...
export function SystemPortal({ system, portalKey, view = "inicio" }: { system: SolarSystem; portalKey: string; view?: SystemPortalView }) {
  ...
  const openRequest = (type: RequestType) => /^[0-9a-f-]{36}$/i.test(portalKey) ? setRequestType(type) : notify("Modo demostración: escanea el QR de una vivienda para enviar solicitudes");
  return <div className="min-h-[100dvh] bg-[#faf9f6] text-stone-900"><Header id={system.id} portalKey={portalKey} /><main className="mx-auto min-h-[70vh] max-w-6xl space-y-12 px-5 pb-32 pt-8 md:px-8 md:pb-16 md:pt-12">{view === "inicio" && <><SystemSummary system={system} /><NextMaintenance system={system} onRequest={() => openRequest("maintenance")} /><Observations system={system} /></>}{view === "historial" && <MaintenanceHistory system={system} />}{view === "documentos" && <Documents system={system} notify={notify} />}{view === "soporte" && <QuickActions id={portalKey} openRequest={openRequest} />}</main><footer className="border-t border-stone-200 bg-white px-5 py-10 text-center text-xs text-stone-400"><div className="mb-2 flex items-center justify-center gap-2 font-semibold tracking-[.16em] text-stone-700"><ShieldCheck className="size-4 text-[#9b7835]" /> SOLARIS</div>Expediente de mantenimiento · {system.id}</footer><MobileNavigation id={portalKey} />{requestType && <RequestDialog type={requestType} systemId={system.id} portalKey={portalKey} close={() => setRequestType(null)} completed={notify} />}{toast && <div role="status" className="fixed bottom-24 left-1/2 z-50 -translate-x-1/2 rounded-full bg-stone-900 px-5 py-3 text-sm font-medium text-white shadow-xl md:bottom-8">{toast}</div>}</div>;
}
