...
function SystemSummary({ system }: { system: SolarSystem }) {
  ...
  return <section id="inicio" className="scroll-mt-6">
    <div className="mb-8 md:mb-10"><div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-[.2em] text-[#9b7835]"><Sparkles className="size-3.5" /> Vivienda vinculada</div><h1 className="max-w-2xl text-3xl font-medium leading-[1.08] tracking-[-.04em] text-[#171713] md:text-5xl">Tu sistema fotovoltaico</h1><p className="mt-4 max-w-xl text-sm leading-6 text-stone-500 md:text-base">Expediente, servicios y documentos asociados al QR de esta vivienda.</p></div>
...
function Documents({ system, notify }: { system: SolarSystem; notify: (message: string) => void }) {
  ...
  return <section id="documentos" className="scroll-mt-8"><SectionHeading eyebrow="Expediente digital" title="Documentos del sistema" />{system.documents.length ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{system.documents.map((doc, index) => { const Icon = icons[index % icons.length]; return <button key={`${doc.name}-${index}`} onClick={() => notify("Disponible próximamente")} className="group flex min-h-28 items-center gap-4 rounded-3xl border border-stone-200 bg-white p-5 text-left transition hover:-translate-y-0.5 hover:border-[#c8a65c] hover:shadow-lg"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-[#f4efe4] text-[#9b7835]"><Icon className="size-5" /></span><span className="min-w-0 flex-1"><span className="block text-sm font-semibold text-stone-900">{doc.name}</span><span className="mt-1 block text-xs text-stone-400">{doc.type}</span></span><ChevronRight className="size-4 text-stone-300 transition group-hover:translate-x-0.5 group-hover:text-[#9b7835]" /></button>; })}</div> : <EmptyState icon={FolderOpen} title="Aún no hay documentos disponibles" description="Los reportes, diagramas y fotografías aparecerán después de ser cargados por el equipo técnico." />}</section>;
}
...
function QuickActions({ id, openRequest }: { id: string; openRequest: (type: RequestType) => void }) {
  ...
  return <section><SectionHeading eyebrow="Acciones principales" title="¿Qué necesitas?" /><div className="grid gap-3 sm:grid-cols-2">{actions.map(({ label, description, icon: Icon, action }) => <button key={label} onClick={() => openRequest(action)} className="group flex min-h-40 flex-col justify-between rounded-3xl border border-stone-200 bg-white p-6 text-left transition hover:-translate-y-0.5 hover:border-[#c8a65c] hover:shadow-lg"><span className="grid size-11 place-items-center rounded-2xl bg-[#f4efe4] text-[#9b7835]"><Icon className="size-5" /></span><span className="mt-8"><span className="block text-base font-semibold leading-5 text-stone-900">{label}</span><span className="mt-2 block text-sm text-stone-500">{description}</span></span></button>)}</div><div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-3 text-sm"><span className="text-stone-400">También puedes consultar:</span><Link href={portalHref(id, "historial")} className="font-medium text-stone-600 underline decoration-stone-300 underline-offset-4 hover:text-[#8a682c]">Historial</Link><Link href={portalHref(id, "documentos")} className="font-medium text-stone-600 underline decoration-stone-300 underline-offset-4 hover:text-[#8a682c]">Documentos</Link></div></section>;
}

type RequestType = "maintenance" | "failure";

function RequestDialog({ type, systemId, portalKey, demoMode, close, completed }: { type: RequestType; systemId: string; portalKey: string; demoMode: boolean; close: () => void; completed: (message: string) => void }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (demoMode) {
        ...
        setReference(`DEMO-${type === "maintenance" ? "MANT" : "FALLA"}-${Date.now().toString().slice(-6)}`);
        return;
      }
