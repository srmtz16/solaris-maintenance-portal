...
const navItems = [
  ...
  { label: "Mi sistema", icon: Gauge, section: "sistema" },
...
function SystemSummary({ system }: { system: SolarSystem }) {
  ...
  return <section id="inicio" className="solaris-summary scroll-mt-6">
    <div className="mb-8 md:mb-10"><div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-[.2em] text-[#8A6200]"><Sparkles className="size-3.5" /> Vivienda vinculada</div><h1 className="max-w-3xl text-3xl font-medium leading-[1.08] tracking-[-.04em] text-[#06131B] md:text-5xl">{system.clientName ? <>{system.welcomeLabel}, {system.clientName}</> : "Tu sistema fotovoltaico"}</h1><p className="mt-4 max-w-xl text-sm leading-6 text-stone-500 md:text-base">Expediente, servicios y documentos asociados al QR de esta vivienda.</p></div>
    <div className="solaris-metrics grid overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-[0_18px_50px_rgba(28,25,20,.06)] sm:grid-cols-2 lg:grid-cols-4">
...
function NextMaintenance({ system, onRequest }: { system: SolarSystem; onRequest: (type: RequestType) => void }) {
  return <section className="solaris-maintenance relative overflow-hidden rounded-[2rem] bg-[#06131B] p-6 text-white shadow-[0_20px_60px_rgba(20,20,15,.16)] md:p-10">
    <div className="absolute -right-16 -top-20 size-56 rounded-full bg-[#F4B400]/15 blur-2xl" /><div className="relative grid gap-8 md:grid-cols-[1fr_auto] md:items-end"><div><div className="mb-5 flex size-11 items-center justify-center rounded-2xl border border-white/10 bg-white/5 text-[#F4B400]"><CalendarDays className="size-5" /></div><p className="text-xs font-semibold uppercase tracking-[.2em] text-[#F4B400]">Próximo mantenimiento</p><h2 className="mt-2 text-3xl font-medium tracking-[-.03em] md:text-4xl">{system.nextMaintenance}</h2><p className="mt-3 text-sm text-stone-400">Mantenimiento preventivo recomendado</p></div><div className="grid gap-3 sm:grid-cols-2 md:min-w-[420px]"><button onClick={() => onRequest("maintenance")} className="flex h-12 items-center justify-center gap-2 rounded-full bg-[#F4B400] px-6 text-sm font-semibold text-[#06131B] transition hover:bg-[#FFD966]">Solicitar mantenimiento <ArrowRight className="size-4" /></button><button onClick={() => onRequest("failure")} className="flex h-12 items-center justify-center gap-2 rounded-full border border-white/15 bg-white/5 px-6 text-sm font-semibold text-white transition hover:bg-white/10"><Headphones className="size-4" /> Reportar falla</button></div></div>
  </section>;
}
...
function MaintenanceHistory({ system, portalKey, notify }: { system: SolarSystem; portalKey: string; notify: (message: string) => void }) {
  ...
  const chipClass = "inline-flex min-h-9 items-center gap-1.5 rounded-full border border-stone-200 bg-stone-50 px-3 py-2 text-xs font-medium text-stone-600 transition hover:border-[#F4B400] hover:bg-[#FFF6D9] hover:text-[#8A6200] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#F4B400]";
  return <section id="historial" className="scroll-mt-8"><SectionHeading eyebrow="Trazabilidad" title="Historial de mantenimiento" /><div className="space-y-4">{system.maintenanceHistory.map((item) => <article key={item.id || `${item.date}-${item.type}`} className="group rounded-3xl border border-stone-200 bg-white p-5 transition hover:border-stone-300 md:p-6"><div className="grid gap-5 sm:grid-cols-[112px_1fr_auto] sm:items-center"><div><div className="text-sm font-semibold text-stone-900">{item.date}</div><div className="mt-1 text-xs text-stone-400">Fecha de servicio</div></div><div className="sm:border-l sm:border-stone-100 sm:pl-6"><h3 className="font-semibold text-stone-900">{item.type}</h3>{item.work && <p className="mt-3 whitespace-pre-wrap text-sm text-stone-600">{item.work}</p>}{item.findings && <p className="mt-2 whitespace-pre-wrap text-sm text-stone-600"><strong>Observaciones:</strong> {item.findings}</p>}{item.recommendations && <p className="mt-2 whitespace-pre-wrap text-sm text-stone-600"><strong>Recomendaciones:</strong> {item.recommendations}</p>}<div className="mt-3 flex flex-wrap gap-2"><Link href={portalHref(portalKey, "documentos")} className={chipClass}><FileText className="size-3.5" /> Reporte</Link><Link href={portalHref(portalKey, "documentos")} className={chipClass}><Camera className="size-3.5" /> Fotografías</Link><button type="button" onClick={() => notify([item.work, item.findings, item.recommendations].filter(Boolean).join(" · ") || "Este servicio no tiene observaciones registradas")} className={chipClass}><MessageSquareText className="size-3.5" /> Observaciones</button><Link href={portalHref(portalKey, "soporte")} className={chipClass}><Wrench className="size-3.5" /> {item.technician}</Link></div></div><div className="flex items-center gap-2 text-xs font-semibold text-emerald-700"><span className="grid size-6 place-items-center rounded-full bg-emerald-50"><Check className="size-3.5" /></span>{item.status}</div></div></article>)}</div></section>;
}

function TechnicalProfile({ system }: { system: SolarSystem }) {
  const items = [
    ["Cantidad de paneles", system.numPanels],
    ["Potencia por panel", system.panelPower],
    ["Marca de panel", system.panelBrand],
    ["Modelo de inversor", system.inverterModel],
    ["Serie del inversor", system.inverterSerial],
    ["Estado del sistema", system.systemStatus],
  ];
  return <section><SectionHeading eyebrow="Configuración instalada" title="Información de tu sistema" /><p className="-mt-2 mb-6 max-w-2xl text-sm leading-6 text-stone-500">Datos técnicos registrados por Solaris. Esta información es de consulta y solo puede ser actualizada por el administrador.</p><div className="grid overflow-hidden rounded-3xl border border-stone-200 bg-white sm:grid-cols-2 lg:grid-cols-3">{items.map(([label, value], index) => <div key={label} className={`p-5 md:p-6 ${index > 0 ? "border-t border-stone-100 sm:border-t-0 sm:border-l" : ""} ${index >= 2 ? "sm:border-t" : ""} ${index === 2 ? "sm:border-l-0 lg:border-l lg:border-t-0" : ""} ${index === 3 ? "lg:border-l-0 lg:border-t" : ""}`}><p className="text-[11px] font-semibold uppercase tracking-[.14em] text-stone-400">{label}</p><p className="mt-3 text-base font-semibold text-stone-900">{value}</p></div>)}</div><div className="mt-3 flex justify-end"><span className="inline-flex items-center gap-1.5 rounded-full border border-stone-200 bg-white px-3 py-1.5 text-xs font-medium text-stone-500"><ShieldCheck className="size-3.5 text-[#8A6200]" /> Solo lectura</span></div></section>;
}
...
function QuickActions({ id, openRequest }: { id: string; openRequest: (type: RequestType) => void }) {
  ...
  const cardClass = "group flex min-h-40 flex-col justify-between rounded-3xl border border-stone-200 bg-white p-6 text-left transition hover:-translate-y-0.5 hover:border-[#F4B400] hover:shadow-lg focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#F4B400]";
  return <section><SectionHeading eyebrow="Acciones principales" title="¿Qué necesitas?" /><div className="grid gap-3 sm:grid-cols-2">{actions.map(({ label, description, icon: Icon, action }) => <button key={label} onClick={() => openRequest(action)} className={cardClass}><span className="grid size-11 place-items-center rounded-2xl bg-[#FFF6D9] text-[#8A6200]"><Icon className="size-5" /></span><span className="mt-8"><span className="block text-base font-semibold leading-5 text-stone-900">{label}</span><span className="mt-2 block text-sm text-stone-500">{description}</span></span></button>)}{consultations.map(({ label, description, icon: Icon, href }) => <Link key={label} href={href} className={cardClass}><span className="grid size-11 place-items-center rounded-2xl bg-[#FFF6D9] text-[#8A6200]"><Icon className="size-5" /></span><span className="mt-8"><span className="flex items-center justify-between gap-3 text-base font-semibold leading-5 text-stone-900">{label}<ChevronRight className="size-4 text-stone-300 transition group-hover:translate-x-0.5 group-hover:text-[#8A6200]" /></span><span className="mt-2 block text-sm text-stone-500">{description}</span></span></Link>)}</div></section>;
}
