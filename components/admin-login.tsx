...
export function AdminLogin() {
  ...
  return <main className="grid min-h-screen place-items-center bg-[#f4f0e8] px-5 py-10 text-stone-900">
    <section className="w-full max-w-md rounded-[2rem] border border-stone-200 bg-white p-7 shadow-[0_24px_70px_rgba(28,25,20,.10)] sm:p-10">
      <div className="grid size-12 place-items-center rounded-2xl bg-stone-900 text-[#d6b76f]"><Sun className="size-6" /></div>
      <p className="mt-7 text-xs font-semibold uppercase tracking-[.2em] text-[#9b7835]">Solaris</p>
      ...
      <form onSubmit={submit} className="mt-8 space-y-5">
        <label className="block text-sm font-medium">Correo<input required type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-stone-200 px-4 outline-none focus:border-[#9b7835]" /></label>
        <label className="block text-sm font-medium">Contraseña<input required type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-stone-200 px-4 outline-none focus:border-[#9b7835]" /></label>
